...
Func _TCPServer_Start($iPort, $sInterface = '0.0.0.0')
	...
	fsdebug("Listening status: " & $__TCPServer_MainSocket)
	If @error Or $__TCPServer_MainSocket <= 0 Then Return SetError(@error, @extended, False)
...
Func __TCPServer_Accept()
	If $__TCPServer_Sockets[0] >= $_TCPServer_MaxClients Then
		fsdebug("Max clients reached")
		Return
	EndIf
	$accept = TCPAccept($__TCPServer_MainSocket)
	If $accept = -1 Then
		fsdebug("No clients connected (" & @error & ")")
		Return
	EndIf
	fsdebug("Someone's connected")
	For $i = 1 To $_TCPServer_MaxClients
